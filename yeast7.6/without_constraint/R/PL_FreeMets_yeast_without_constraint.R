...
currency_met <- function (mod3,indx) {
    ...
    EbioFree<-biomass_reactsADDALL1(free_model,match(react_id(yeast)[2984],react_id(free_model)))
...
for(k in 1: 11) {
    ...
    for(f in 1:length(fluxPercent)){
        ...
        EplSumFree<-rowSums(run_pl(unname(wts),RNX_idF,FreeMod[[1]],Emodel,fluxPercent[f],seq(1,length(allGenes(mod2))),fdWT))
